angular.module('TrumpMEME.factories', [])

.factory('InitialRun', function($window, RedditService, GiphyService, ImgurService) {
  return {
    setInitialRun: function(initial) {
      console.log('factory setInitialRun');
      $window.localStorage.initialRun = (initial ? "true" : "false");
      //AWS GET REDDIT CLIENTID
      RedditService.getRedditClientId(function(data) {
        console.log('data ' + JSON.stringify(data.Item.ClientId.S));
        $window.localStorage.redditClientId = JSON.stringify(data.Item.ClientId.S);
        $window.localStorage.redditClientId = $window.localStorage.redditClientId.replace(/"/g, "");
        console.log('data ' + JSON.stringify(data.Item.Secret.S));
        $window.localStorage.redditSecret = JSON.stringify(data.Item.Secret.S);
        $window.localStorage.redditSecret = $window.localStorage.redditSecret.replace(/"/g, "");
      });
      console.log('$window.localStorage.redditClientId ' + $window.localStorage.redditClientId);
      //AWS GET GIPHY API KEY
      GiphyService.getGiphyApiKey(function(data) {
        console.log('data ' + JSON.stringify(data.Item.ApiKey.S));
        $window.localStorage.giphyApiKey = JSON.stringify(data.Item.ApiKey.S);
        $window.localStorage.giphyApiKey = $window.localStorage.giphyApiKey.replace(/"/g, "");
      });
      console.log('$window.localStorage.giphyApiKey ' + $window.localStorage.giphyApiKey);
      //AWS GET IMGUR API KEY
      ImgurService.getImgurClientId(function(data) {
        console.log('data.Item.ClientId.S ' + data.Item.ClientId.S);
        $window.localStorage.imgurClientId = data.Item.ClientId.S;
        $window.localStorage.imgurClientId = $window.localStorage.imgurClientId.replace(/"/g, "");
      });
      console.log('$window.localStorage.imgurClientId ' + $window.localStorage.imgurClientId);
      console.log('self.clientIdsCache ' + self.clientIdsCache);
    },
    isInitialRun: function() {
      var value = $window.localStorage.initialRun || "true";
      console.log('factory isInitialRun');
      return value == "true";
    }
  };
})
